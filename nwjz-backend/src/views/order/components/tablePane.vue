  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link style="margin-right:5px;" :to="{ path:'create'}">
        <el-button class="filter-item" style="margin-left: 5px;" type="success" icon="el-icon-plus">新增合同</el-button>
      </router-link>
    </div>
    <el-table :key="tableKey" v-loading="listLoading" :border="true" :data="list" element-loading-text="给我一点时间" stripe fit highlight-current-row style="width: 100%">
      <el-table-column align="center" prop="name" label="客户姓名" width="80">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="100px" label="客户电话">
        <template slot-scope="scope">
          <span>{{ scope.row.contact_phone }}</span>
        </template>
      </el-table-column>
      <el-table-column align="left" label="客户地址" min-width="150px">
        <template slot-scope="scope">
          <span>{{ scope.row.address }}</span>
        </template>
      </el-table-column>
      <el-table-column align="left" label="阿姨姓名" min-width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.remark }}</span>
        </template>
      </el-table-column>
      <el-table-column align="left" label="阿姨工资" min-width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.remark }}</span>
        </template>
      </el-table-column>
      <el-table-column align="left" label="合同起始时间" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.remark }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="80px" label="签单人">
        <template slot-scope="scope">

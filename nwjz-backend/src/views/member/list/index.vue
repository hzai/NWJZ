  ...
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" element-loading-text="给我一点时间" stripe fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="头像" width="65">
        <template slot-scope="scope">
          <router-link :to="'detail?userId='+scope.row._id"><img :src="scope.row.avatar" class="user-avatar"></router-link>
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="150px" label="联系电话">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" min-width="100" class-name="small-padding" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" icon="el-icon-edit" @click="goInfo(scope.row)" />
        </template>
      </el-table-column>
    </el-table>

<template>
  <div class="app-container">
    ...
    <el-table :data="List" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="center" label="保险单号" width="200">
      ...
      <el-table-column align="center" label="受保人" width="80">
        <template slot-scope="scope">
          {{ scope.row.insurance_type }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="开始时间/结束时间" width="180">
        <template slot-scope="scope">
          <span>{{ new Date(scope.row.start_date) | parseTime('{y}-{m}-{d}') }}</span><br>
          <span>{{ new Date(scope.row.end_date) | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" top="5vh" center :title="dialogType==='edit'?'编辑保险记录':'新增保险记录'">
      <div style="margin:0 auto;">
        <el-form ref="postForm" :model="model" label-width="130px" label-position="right" :rules="rules">
          <el-form-item label="保险单号" prop="insurance_no">
            <el-input v-model="model.insurance_no" placeholder="保险单号" style="width:300px;" />
          </el-form-item>
          <el-form-item label="保险类型" prop="insurance_type">
            <el-select v-model="model.insurance_type" placeholder="请选择" style="width:300px;">
              <el-option v-for="(item, key) in insuranceTypeOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="受保人" prop="insurant">
            <el-input v-model="model.insurant" placeholder="输入姓名或手机查询" style="width:300px;" />
          </el-form-item>
          <el-form-item label="可以换人" prop="can_changed">
            <el-switch v-model="model.can_changed" active-text="是" inactive-text="否" />
          </el-form-item>
          <el-form-item label="开始时间" prop="start_date">
            <el-date-picker v-model="model.start_date" type="datetime" default-time="00:00:00" placeholder="选择时间" style="width:300px;" />
          </el-form-item>
          <el-form-item label="结束时间" prop="end_date">
            <el-date-picker v-model="model.end_date" type="datetime" default-time="23:59:59" placeholder="选择时间" style="width:300px;" />
          </el-form-item>
          <el-form-item label="状态" prop="status">
            <el-select v-model="model.status" placeholder="请选择" style="width:300px;">
